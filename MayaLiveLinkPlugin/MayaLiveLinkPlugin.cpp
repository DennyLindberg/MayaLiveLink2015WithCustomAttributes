...
struct FLiveLinkStreamedJointHeirarchySubject : IStreamedEntity
{
	...
	/*
		Queries keyframed user defined attributes from JointsToStream[0] (Root joint).
		LiveLinkCurves are updated with the attribute short name and its interpreted float value.
	*/
	void UpdateCustomCurvesFromRootJoint(TArray<FLiveLinkCurveElement>& Curves)
	{
		...
		// TODO: Get attributes that does not have keyframes on them. getConnections depends on attribute having keyframes.
		MPlugArray plugs;
		JointsToStream[0].JointObject.getConnections(plugs);
		for (unsigned int i = 0; i < plugs.length(); i++)
		{
			if (plugs[i].isKeyable() && plugs[i].isDynamic()) // isDynamic() identifies a custom attribute when it has keyframes applied
			{
				int index = Curves.AddDefaulted();
				Curves[index].CurveName  = FName(plugs[i].partialName().asChar());
				Curves[index].CurveValue = plugs[i].asFloat();

				// Enable this to print debug values in Maya
				//MGlobal::displayInfo(MString("Sending ") + MString(nodePlugs[i].name()) + MString(": ") + plug.asFloat());
			}
		}
	}
